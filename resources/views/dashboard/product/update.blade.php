...
<div class="col-12 grid-margin stretch-card">
    ...
    <input type="text" class="form-control" id="exampleInputName1" value="{{ $data->name }}" placeholder="Name" name="name">
    </div>
    @error('price')<div style="color:red;">{{ $message }} </div>@enderror
    <div class="form-group">
    <label for="exampleInputEmail3">price</label>
    <input type="text" class="form-control" id="exampleInputEmail3" value="{{ $data->price }}" placeholder="price" name="price">
    </div>
    @error('sale')<div style="color:red;">{{ $message }} </div>@enderror
    <div class="form-group">
    <label for="exampleInputPassword4">sale</label>
    <input type="text" class="form-control" id="exampleInputPassword4" value="{{ $data->sale }}" name="sale" placeholder="sale">
    </div>
    @error('count')<div style="color:red;">{{ $message }} </div>@enderror
    <div class="form-group">
    <label for="exampleInputPassword4">count </label>
    <input type="text" class="form-control" id="exampleInputPassword4" value="{{ $data->count }} " name="count" placeholder="count">
    </div>
    <div class="form-group">
    <label for="exampleSelectGender">category</label>
    <select name="category" class="form-control" id="exampleSelectGender">
        @foreach (config("cat") as $val)
        <option value="{{ $val }}" @selected($val==$data->category) >{{ $val }}</option>
